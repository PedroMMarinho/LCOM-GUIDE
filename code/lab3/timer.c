...
int timer_hook_id = 0; // Timer hook_id needs to be unique
...
int (timer_subscribe_int)(uint8_t *bit_no) {
  ...
  *bit_no = BIT(timer_hook_id); // This value will change after setting the policy
  if(sys_irqsetpolicy(TIMER0_IRQ, IRQ_REENABLE, &timer_hook_id) != OK) // Set correct policy
    return 1;
...
int (timer_unsubscribe_int)() {
  if(sys_irqrmpolicy(&timer_hook_id) != OK) // Remove the policy
    return 1;
  return 0;
}
